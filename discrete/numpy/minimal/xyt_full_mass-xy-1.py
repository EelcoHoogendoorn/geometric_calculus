...
def leapfrog(phi):
	...
	edt(s, -(+idx(xt) + idy(yt) + m * interpolate(xyt, +1, +1) + M * t))  # t
	edt(x, +(+edx(t) - idy(xyt) - m * interpolate(yt, -1, +1) + M * xt))  # xt
	edt(y, +(+idx(xyt) + edy(t) + m * interpolate(xt, +1, -1) + M * yt))  # yt
	edt(xy, -(+edx(yt) - edy(xt) - m * interpolate(t, -1, -1) + M * xyt))  # xyt
	idt(t, +(+idx(x) + idy(y) + m * interpolate(xy, +1, +1) + M * s))  # s
	idt(xt, -(+edx(s) - idy(xy) - m * interpolate(y, -1, +1) + M * x))  # x
	idt(yt, -(+idx(xy) + edy(s) + m * interpolate(x, +1, -1) + M * y))  # y
	idt(xyt, +(+edx(y) - edy(x) - m * interpolate(s, -1, -1) + M * xy))  # xy
